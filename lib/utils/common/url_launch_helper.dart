...
import 'package:design/utils/common/app_string.dart';
...
class AppUrlLauncher {
  static Future<void> launchUrlFun({required String? url}) async {
    ...
    try {
      final Uri uri = (!url.startsWith("https"))
          ? Uri.parse("https://$url")
          : Uri.parse(url);
      await launchUrl(uri);
    } catch (e) {
  ...
  static Future<void> openBrowserLaunch(String uri) async {
    try {
      final url = (!uri.startsWith("https"))
          ? Uri.parse("https://$uri")
          : Uri.parse(uri);
      final isNotLaunch = await canLaunchUrl(url).whenComplete(() {});
      if (!isNotLaunch) {
  ...
  static Future<void> openWhatsappLaunch(String phoneNumber,
      {String? message}) async {
    try {
      var contact = phoneNumber;
      // var value = message??AppEndPoint.baseUrl;
      FocusManager.instance.primaryFocus?.unfocus();
      var whatsappAndroid = Uri.parse("whatsapp://send?phone=$contact" +
          // "&text=${Uri.encodeComponent(value)}"
          contact);
      var whatsappIos = Uri.parse(
          // "https://wa.me/$contact/?text=${Uri.parse(value)}",
